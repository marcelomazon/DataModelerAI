...
const Sandbox: React.FC<SandboxProps> = ({
  caseStudy, entities, setEntities, relationships, setRelationships, onEvaluate, isEvaluating, onBack, quota, onQuotaUpdate
}) => {
  ...
  const handleGenerateSQL = async (dbType: DatabaseType) => {
    ...
    if (!canUseSQL()) {
      alert("Você atingiu o limite de geração de scripts SQL diário (3 usos). Tente novamente amanhã ou configure sua própria chave de API.");
      return;
    }
  ...
  const handleGetHint = async () => {
    if (!canUseMentor()) {
      alert("Você atingiu o limite diário dicas do Mentor IA. Tente novamente amanhã ou configure sua própria chave da API.");
      return;
    }
          ...
          <div className="relative" ref={dbMenuRef}>
            ...
            >
              ...
              <div className="hidden xl:flex flex-col items-start leading-tight">
                <span className="text-xs font-bold text-slate-600">SQL DDL</span>
                <span className="text-[9px] opacity-80 font-normal">({quota.sqlUses}/{MAX_SQL_USES})</span>
              </div>
